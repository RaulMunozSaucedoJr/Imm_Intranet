...
if (!function_exists('admin_toastr')) {
    ...
    {
        // Opciones predeterminadas que incluyen la duración
        $defaultOptions = [
            'duration' => 5000,
        ];

        // Fusionar las opciones proporcionadas con las predeterminadas
        $options = array_merge($defaultOptions, $options);

        // Agregar las opciones gravity y position a los parámetros
        $gravity = isset($options['gravity']) ? $options['gravity'] : 'bottom';
        $position = isset($options['position']) ? $options['position'] : 'center';

        // Crear un array con los valores proporcionados
        $toastrData = [
            'message' => $message,
            'type' => $type,
            'options' => [
                'gravity' => $gravity,
                'position' => $position,
                'duration' => $options['duration'], // Incluir la duración
            ],
        ];

        // Crear un nuevo MessageBag
        $toastr = new MessageBag($toastrData);
